...
Bellow please find a abbreviations table used in this report that will make it easier to understand:
\begin{table}[H]
  \centering
...
Each new observation $y_k$ at step $k$ reweights particles, carried out entirely
in the log domain:
\begin{equation}
...
$N_{\mathrm{eff}} < 0.75N$, an aggressive threshold chosen deliberately, which when tested worked best.
...
\begin{enumerate}
  ...
  environment steps are gathered per update - the sb3-contrib default, left unoverridden here.
...
be evaluated at any point, and a crash costs at most one interval.
...
single-hidden-layer time policy of 16 units. This takes about 45 minutes on CPU, roughly 27\,s per
generation.
...
Both flatten out well before the 125th measurement. The flattening is the
...
$N = 125$, not a particle-filter failure as ESS stays near 1960 of 2000.
...
Bellow are presented interrogation times chosen by the two policies, 
plotted against the measurement step. The TRPO policy chooses times 
that are generally shorter than those chosen by the CEM policy as 
TRPO settles around \number{4} while CEM settles around \number{6.7}.
\begin{figure}[H]
  \centering
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
...
Bayesian experimental design for single-qubit Ramsey frequency estimation. Sequential Monte
Carlo filter that is used to calcualte posterior for prior. Trained policy is used to select
interrogation time $t$ for the next shot based on posterior. Next measurement outcome corrects
...
\emph{Neural-network heuristics for adaptive Bayesian quantum estimation}. Two learned policies
were built on the same Gymnasium environment and evaluated on \num{10000} randomly selected
$\omega$. TRPO and CEM reach similar performance of ($2.173\times10^{-3}$ vs $2.137\times10^{-3}$
